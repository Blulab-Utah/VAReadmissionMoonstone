...
public class StartupParameters {
	...
	private void readProperties(String propertyFileName) throws TSLException {
		...
		int lindex = fpath.lastIndexOf(File.separatorChar);
		String rstr = fpath.substring(0, lindex);
		this.rootDirectory = rstr;
		try {
		...
		if (!(new File(this.ruleDirectory).isDirectory()) || !(new File(this.resourceDirectory).isDirectory())
